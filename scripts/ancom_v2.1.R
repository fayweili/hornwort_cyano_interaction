...
# Data Pre-Processing
feature_table_pre_process = function(feature_table, meta_data, group_var = NULL, 
  ...
  sample_ID = intersect(rownames(meta_data), colnames(feature_table))
  feature_table = feature_table[, sample_ID]
  meta_data = meta_data[match(sample_ID, rownames(meta_data)), ]
...
ANCOM = function(feature_table, meta_data, struc_zero = NULL, main_var, p_adj_method = "BH", 
  ...
  res = list(out = out, fig = fig, eff_size = eff_size)
  return(res)
}
